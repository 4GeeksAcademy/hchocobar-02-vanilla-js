...
window.onload = function() {
  ...
  users.map((item, index) => {
    textHTML += `
    <li class="list-group-item">${item.name} - ${item.company.name}</li>
    `
    // console.log(textHTML);
  })
  // Capturando el elemento HTML
  let tag = document.querySelector('#list-users')
  tag.innerHTML = textHTML

  /* Renderizando una Card */
  let cardHTML = ''
  users.map((item) => {
    cardHTML += `
      <div class="card">
        <img src="https://randomuser.me/api/portraits/women/${item.id + 40}.jpg" class="card-img-top" alt="Photo ${item.name}">
        <div class="card-body">
